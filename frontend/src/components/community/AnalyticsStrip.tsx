...
import { ANALYTICS } from '@/src/data/communityMockData';
...
export default function AnalyticsStrip({ onViewDetails }: AnalyticsStripProps) {
  return (
    <div className="gfg-analytics-strip flex flex-wrap items-center gap-3 sm:gap-4">
      {ITEMS.map((item) => (
        <button
          key={item.label}
          type="button"
      ...
      <button
        type="button"
        onClick={onViewDetails}
        className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[#F0FDF4] dark:bg-[rgba(34,197,94,0.15)] border border-[#2F8D46]/30 dark:border-[rgba(34,197,94,0.35)] hover:bg-[#F0FDF4]/90 dark:hover:bg-[rgba(34,197,94,0.2)] transition-colors"
        title="View detailed analytics"
      >
